...
def clean_sg(region, cluster_name):
    ...
    if not region:
        try:
            metadata = urllib.request.urlopen(
                'http://169.254.169.254/latest/dynamic/instance-identity/document', timeout=2).read().decode()
            region = json.loads(metadata)['region']
        except:
            region = boto3.session.Session().region_name
            logging.warning(
                'Cannot detect region from metadata, use default region({})'.format(region))
    ...
    if not cluster_name:
        try:
            ...
            logging.error(
                'Cannot load cluster name from kubeconfig or cluster name option not specify')
            exit(1)
    ...
    if sg_to_delete:
        logging.info("{} - {} sg are currently in use by k8s services, {} will be deleted.".format(
            cluster_name, len(elb_sgs), len(sg_to_delete)))
        ...
        for sg in sg_to_delete:
            ...
            try:
                ...
                logging.error(
                    'Unable to delete following SG : {}: {}'.format(sg, e))
    else:
        logging.info('There is no leaked security group')
